...
### I.懒汉式(线程不安全) 
在这种实现中，私有静态变量`instance`被延迟实例化。这个实现在多线程环境下是不安全的，如果多个线程同时进入`if(instance == null)`，并且此时`instance`为`null`，那么会有多个线程执行`instance = new Singleton();`语句，将会导致`instance`被实例化多次。
```
...
### V.静态内部类实现(线程安全)
当`Singleton`类被加载时，它的静态内部类不会被加载进内存。只有当调用`getInstance()`方法从而触发`SingletonHolder.INSTANCE`时`SingletonHolder`才会被加载，并初始化`INSTANCE`实例。
```
public class Singleton {
    ...
    public static Singleton getInstance() {
        return SingletonHolder.InNSTANCE;
    }
}
```
