...
`00 2F`对应的十进制为15+2*16=47，即常量池数组中有46个元素。
	- 常量1：`0A`即十进制的10，查表得出对应的常量类型为`CONSTANT_Methodref_info`，类中方法的符号引用，接下来2个字节`00 0A`对应的十进制数值为10，表示声明此方法的类描述符的索引项为常量池数组中的第10个常量；再向后2个字节`00 22`对应十进制数34，表示指向此类的名称